  ...
  const CHANGELOG: { version: string; notes: string[] }[] = [
    {
      ...
      notes: [
        ...
        "The terminal wizard no longer drops the contacts screen without saying why: it tells you when there is no DM activity on record yet, or when the record simply has nothing new. Each screen also says when there is more than it can show - `Only showing 5 of 23 (most recent)` - and how many archived groups it left out.",
        ...
        "`/whatsapp-channel:access review` is no longer a list you page through four at a time: it opens with what you already have and what is on offer, shows groups and contacts on one screen, and lets you type part of a name to search everything cached instead of scrolling. Adding and removing now happen in the same pass, and it shows you the whole +/- list and waits for you to say Apply before it writes anything. Contacts you only know through a group you granted roster access to are offered too, so you no longer have to wait for someone to DM you before you can allowlist them.",
        "Changed your mind: `/whatsapp-channel:access undo` puts back the access list from just before the last review or manage run - run it with `--dry-run` first to see exactly what would come back and what would go. Cached names are not restored, only the access list itself.",
        "The terminal wizard is now one screen per kind instead of two separate commands: what Claude can already reach comes pre-ticked, so you untick to take access away and tick to grant, in the same pass. It shows you the complete list of additions and removals and waits for you to approve it before writing anything, and `wizard --undo` puts the last run back. `wizard --revoke` still works - it opens that same screen.",
