  ...
  const CHANGELOG: { version: string; notes: string[] }[] = [
    ...
    },
    {
      ...
      notes: [
        ...
        'Replies you type on your phone are no longer invisible to Claude. For chats already on your allowlist, the message is logged the moment you send it, so the unreplied list clears and `catch_up` shows both sides of the conversation instead of only Claude\'s half. Your own words stay readable for an hour and then collapse to a "replied (text expired)" line, and `catch_up` now replays the last 5 messages from each side of a chat rather than 15 in total.',
        ...
        'Groups with mention required: a member message that does not mention Claude is now kept in the local log (text only) and shown by catch_up as "not addressed to Claude", so you can ask for a message to that group with the room in view. It is never routed, never notified and never counted as unreplied. catch_up also keeps the last 5 lines from each side of a chat instead of 5 in total, so your own texts cannot push out what others said. Context lines - your own texts, replies sent for you, and unaddressed group chatter - are now kept for 7 days; an unanswered message addressed to Claude still ages out after 24 hours, and the backlog WhatsApp delivers when the plugin reconnects after being offline is now logged too (never acted on). catch_up takes an optional `chat` so a session can pull the recent messages of one chat before drafting a reply, instead of dumping every chat.',
      ],
    },
    {
      version: "0.21.2",
